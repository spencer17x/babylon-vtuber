import {
Animation,
AnimationGroup, Bone, Matrix, 	Node, Quaternion, 	Tools as BabylonTools, Tools,
...
export class FBXAnimations {
	...
	private static _AddClip(animationGroup: AnimationGroup, layer: IFBXLayerCurve, cachedModels: INumberDictionary<Node>, cachedSkeletons: INumberDictionary<IFBXSkeleton>): void {
		...
		const initialScale = scaling.asArray();
		const initialPosition = position.asArray();
		const initialRotation = rotation.toEulerAngles().asArray();
