import type { IndicesArray, Nullable } from '@babylonjs/core/types';
import { Deferred } from '@babylonjs/core/Misc/deferred';
import { Quaternion, Vector3, Matrix, TmpVectors } from '@babylonjs/core/Maths/math.vector';
...
import type { IProperty } from '../glTF2Interface';
import {
	AccessorType,
	CameraType,
...
export class GLTFLoader implements IGLTFLoader {
	 ...
	 * @internal
	 */
	public loadAsync(scene: Scene, data: IGLTFLoaderData, rootUrl: string, _onProgress?: (event: ISceneLoaderProgressEvent) => void, fileName = ''): Promise<void> {
		return Promise.resolve().then(() => {
			this._babylonScene = scene;
			this._loadData(data);
	 ...
	 */
	public loadNodeAsync(context: string, node: INode, assign: (babylonTransformNode: TransformNode) => void = () => {
	...
	private _loadMorphTargetVertexDataAsync(context: string, babylonGeometry: Geometry, attributes: {
		[name: string]: number
	}, babylonMorphTarget: MorphTarget): Promise<void> {
		const promises = new Array<Promise<any>>();
	...
	private _loadBone(node: INode, skin: ISkin, babylonSkeleton: Skeleton, babylonBones: {
		[index: number]: Bone
	}): Bone {
	 ...
	 */
	public loadCameraAsync(context: string, camera: ICamera, assign: (babylonCamera: Camera) => void = () => {
	}): Promise<Camera> {
	 ...
	 */
	public loadTextureInfoAsync(context: string, textureInfo: ITextureInfo, assign: (babylonTexture: BaseTexture) => void = () => {
	}): Promise<BaseTexture> {
	 ...
	 * @internal
	 */
	public _loadTextureAsync(context: string, texture: ITexture, assign: (babylonTexture: BaseTexture) => void = () => {
	}): Promise<BaseTexture> {
